...
const FALLBACKS = ["kp-g-series", "kp-thai", "eng-grammar"];

// Reads the whole user_progress table and aggregates it — too expensive to
// redo on every homepage request, so it's cached and revalidated periodically.
export const getPopularCourseIds = unstable_cache(
  ...
  ["popular-courses"],
  { revalidate: 900, tags: ["popular-courses"] }
);
